...
class ServerFailure extends Failure {
  ...
  factory ServerFailure.fromDioError(DioException dioError) {
    ...
    log('DioError: ${dioError.type}, Message: ${dioError.message}');
  ...
  factory ServerFailure.fromResponse(int? statusCode, dynamic response) {
    ...
    log('45- Response statusCode: $statusCode, Response: $response');
    if (response.toString().contains("Null")) {
      return ServerFailure(" No Result  ");
    }
    if (response is Map<String, dynamic>) {
      // Detailed logging of the response structure
      response.forEach((key, value) {
        // log('50- Response key: $key, value: $value');
      });
